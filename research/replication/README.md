...
| [`haystack.py`](./haystack.py) | Builds paired structured / shuffled haystacks at a target token budget; the shuffled side keeps the same paragraphs but permuted with a fixed seed. CLI: `python -m replication.haystack --vault <path> --target-tokens N`. |
| [`run.py`](./run.py) | Driver: loops over (vault × model × size × structuring), grades each cell, writes a JSON report. Offline mode (substring grading) for sanity checks; live mode talks to any OpenAI-compatible endpoint via `MMB_LLM_API_KEY`. |
| [`analyze.py`](./analyze.py) | Renders a Markdown report with the per-(model, size) accuracy table and the structured-minus-shuffled delta. |

## Protocol

For each vault `V` and target context size `C` ∈ {2K, 5K, 10K, 25K, 50K, 100K, 200K}:

1. Sample a *needle* — a one-line factual claim that appears exactly once in `V`.
...
- Position-of-needle sweeps (start / middle / end of haystack) to disentangle "structured-haystack" effects from "lost in the middle".
